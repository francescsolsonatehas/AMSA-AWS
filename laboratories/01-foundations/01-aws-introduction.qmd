---
title: Unit 1 · Executing a Python scrpt in a EC2 instance
lang: en
---
...
6. Executing a Python script in the EC2 instance
...
### Step 6: Connecting to the EC2 instance {#pas-6-connexio-a-la-instancia-ec2}
...
o connect to the EC2 instance, we will use the terminal on our local computer, just as we did at the beginning [Step 3](#pas-3-importació-de-la-clau-pública-a-aws), Open a terminal on your local computer. We will use the `ssh` command to connect to the remote machine. The command will look like this:
...
## Working in EC2{#execucio-en-el-nostre-laboratori-remot}

En aquest apartat es veurà la transferència de fitxers entre la màquina local i la màquina remota. A més, utilitzarem un script de Python per simular una càrrega de treball i generar un gràfic a partir d'un fitxer de dades. 

### Step 1: Remote environment setup on EC2 {#pas-1-configuracio-de-lentorn-remot-a-lec2}
...
9.  Run the script with the following command:
  ...
  ```
At this point, you should see an error indicating that the file `workload.csv` does not exist. This is normal because we haven’t uploaded the data file to the EC2 instance yet.

### Step 2: Transferring the data file to the EC2 instance EC2 {#pas-2-transferencia-del-fitxer-de-dades-a-la-instancia-ec2}

To transfer the `workload.csv` data file to the EC2 instance, we will use SFTP (SSH File Transfer Protocol). SFTP is a secure protocol for transferring files between a local computer and a remote machine. We will use the same SSH key pair we created earlier to authenticate to the EC2 instance.
...
3. Reconnect to the EC2 instance via SSH, and run the Python program again:
