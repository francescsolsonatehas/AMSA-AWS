...
4. Once the database is available, click on the database and go to the tab **Connectivity & security** to view the connection details.
    ...
    * Endpoint: `wordpress.cp66aquygfg3.us-east-1.rds.amazonaws.com`
    * Port: `3306`
...
6. Add a rule to the **Security Group** of the database to allow connections from the EC2 instance.
	...
	6.2. Select our database instance (`wordpress`)
 ...
 7. Connect to the RDS database from the EC2 instance:
    ...
    ```
   hostname is the database saved earlier (`wordpress.cp66aquygfg3.us-east-1.rds.amazonaws.com`) and
   `admin` is the master user created earlier, along with its password. This is:
   
     ```bash
    mysql -h wordpress.cp66aquygfg3.us-east-1.rds.amazonaws.com -u admin -p
    # Torregrossa2030!!
    ```

8. When entering MySQL, at the MySQL promtp, create the database **wordpress** and user **wordpress** with all permissions:
...
9. Follow the steps of the WordPress installation:
    ...
    - Enter the database connection details:
      ...
      - Database Host: the RDS database endpoint (for example: `wordpress.cp66aquygfg3.us-east-1.rds.amazonaws.com`)
