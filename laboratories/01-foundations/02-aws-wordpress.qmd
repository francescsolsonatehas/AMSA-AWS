...
5. To test the connection, we need a MySQL client. The easiest way is to create an EC2 instance and install the MySQL client.
    ...
    *  Leave everything as it is except Key pair name : If you have created a private key pair (as **aws-keypair** in the figure), select it, else select **vockey**. Then click **Launch instance**.
     ...
     We now have a running EC2 instance. This means we have a remote virtual machine on AWS that we can connect to from the **EC2 console** or from our local computer using SSH (explained in ).
     
     Let’s see how to access from the EC2 console.
	...
	You can alternatively connect with the EC2 instance using an ssh connection this way:	 
	 
	```bash
	cd   # we suppose the .ssh directory is located in the home of the user francesc
	ssh -i .ssh/aws-keypair ec2-user@<public-ip>
	``
	We need to get the public-ip. Go to the EC2 dashboard and click `Instances`. Then cllick the AMSA EC2 instance. Copy the `public-ip`.
