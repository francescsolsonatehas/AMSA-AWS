...
> You’ve probably read a lot of stuff on the internet about UEFI. Here is something important you should understand: 95% of it was probably garbage. — [Adam Williamson](https://amsa.lol/weeks/week-1.html#/what-is-the-biosuefi:~:text=Such%20as%20Adam%20Williamson%20says%3A)

## Fase 2: Etapes {.smaller}

1.  **Inicialització del firmware**
...
## Actualització del firmware {.smaller}
...
-   Millorar funcions de gestió d’energia i arrencada segura ([Secure Boot](https://es.wikipedia.org/wiki/Secure_Boot)).
:::
...
-   Alguns fabricants ofereixen mecanismes de recuperació (dual BIOS, [BIOS flashback](https://www.asus.com/es/support/faq/1038568/), [càpsules UEFI](https://www.preprints.org/manuscript/202502.1245/v1)).
-   En entorns de producció, és recomanable validar la nova versió en un entorn de test abans de desplegar-la.
:::
:::::

## Diferències entre BIOS i UEFI (I) {.smaller}
...
## Diferències entre BIOS i UEFI (II) {.smaller}
...
::: {.fragment .callout-important title="Límitació"}
1.  Com MBR utiltiza entrades de 32 bits per blocs lògics (LBA) → $2^{32}$ sectors × $512 B = 2 TB$ aproximadament.
2.  Només suporta 4 particions primàries.
:::
::::
...
## Què és la consola de la UEFI? {.smaller}
...
1.  Mostrar un menú amb les diverses opcions d'arrencada disponibles.
2.  Carregar el nucli (kernel) i l'Initrd (si s'escau) a la memòria principal.
3.  El kernel de linux té una [convenció d'arrencada](https://github.com/torvalds/linux/blob/v4.16/Documentation/x86/boot.txt) on el **bootloader** ha de preparar el mapa de memòria i els registres de la CPU abans de transferir el control al nucli.
